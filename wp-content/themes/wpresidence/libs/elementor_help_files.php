...
function wpestate_render_elementor_search_tab_wrapper($settings,$elementor_this,$post_id){
    ...
    if( is_array($type_data) && count($type_data)>1 ){
        ...
        foreach ($type_data as $key=>$term_id){
                ...
                $tab_items.= '<div class="adv_search_tab_item item_underline_active_'.esc_attr($settings['wpresidence_tab_item_underline_active']).' '.esc_attr($active).' '.esc_attr($term_slug).'" data-term="'.esc_attr($term_slug).'" data-termid="'.esc_attr($tmp).'" data-tax="'.esc_attr($taxonomy_tab).'">
